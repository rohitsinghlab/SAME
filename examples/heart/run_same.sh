#!/bin/bash
# ==============================================================================
# SAME ISS Heart — Main Optimization Script
# ==============================================================================
# Reproduces: Fig 3c (accuracy vs violations sweep), Fig S6 (MS/dp heatmaps)
# Dataset: Human embryonic heart, ~3160 cells, 8 cell types
#
# Usage:
#   bash run_same.sh --dp 10 --knn 8 --ms 1    # single run (paper config)
#   bash run_same.sh --dp 0 --ms 3              # MS=3, dp=0
#
# To reproduce all paper results, run for each combination:
#   MS=1: dp in {0, 1, 5, 10, 25, 50}, knn=8
#   MS=3: dp in {0, 1, 5, 10, 25, 50}, knn=8
#   MS=7: dp in {0, 1, 5, 10, 25, 50}, knn=8
# ==============================================================================
...
print(f"Loaded: {len(refDF)} template, {len(alignDF)} query cells")

# Metacells
mc_align = greedy_triangle_collapse(
    alignDF, cell_type_col='cell_type', original_idx_col='Cell_Num',
    x_col='X', y_col='Y',
    max_metacell_size=${MS}, r_max=${R_MAX}, min_angle_deg=${MIN_ANGLE_DEG},
